...
public class DiagramRenderer
{
    ...
    {
        double bx = layout.BoardOffsetX(panelOnLeft);
        ...
        AppendRightRail(sb, layout, theme, bx);
        AppendBar(sb, layout, theme, bx);
        AppendPoints(sb, layout, theme, bx, panelOnLeft);
        AppendPointNumbers(sb, layout, theme, bx, panelOnLeft);
        ...
        double bx, bool panelOnLeft)
    {
        for (int pt = 1; pt <= 24; pt++)
        {
            ...
            if (pt >= 13) // top — triangles point downward
            {
            ...
            else // bottom — triangles point upward
            {
        ...
        double bx, DiagramRequest request)
    {
        ...
        sb.AppendLine($"""  <text x="{F(cubeX + cubeSize/2)}" y="{F(cubeY + cubeSize/2)}" dominant-baseline="central" text-anchor="middle" font-family="sans-serif" font-size="{F(cubeSize*0.55)}" font-weight="bold" fill="#222">{request.CubeSize}</text>""");
    }
